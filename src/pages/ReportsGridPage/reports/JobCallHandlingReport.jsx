...
const JobCallHandlingReport = (props) => {
  ...
  const { AverageIdleTime, AveragePostTime, AverageTalkingTime } = data || {};
  ...
  const memoizedPieData = React.useMemo(() => {
    ...
    const AverageTalkingTimePercent =
      AverageTalkingTime && total ? ((talkTime / total) * 100).toFixed(0) : 0;
    const AveragePostTimePercent =
      AveragePostTime && total ? ((postTime / total) * 100).toFixed(0) : 0;
    const AverageIdleTimePercent =
      AverageIdleTime && total ? ((idleTime / total) * 100).toFixed(0) : 0;
    return [
      {
  ...
  const renderTableContent = React.useMemo(() => {
    if (data) {
      return Object.keys(data).map((v) => (
        <Table.Row key={v}>
          <Table.Cell>
            <span className={`${v}`}>{keyVariables[v] || v}</span>
          </Table.Cell>
          <Table.Cell>
            {v.includes('Date') ? moment(data[v]).format('DD.MM.YYYY HH:mm:ss') : data[v]}
          </Table.Cell>
        </Table.Row>
      ));
    }
  ...
  const renderPieContent = React.useMemo(
    () =>
      memoizedPieData.map((entry, index) => <Cell key={`cell-${entry}`} fill={COLORS[index]} />),
    [memoizedPieData]
  );
