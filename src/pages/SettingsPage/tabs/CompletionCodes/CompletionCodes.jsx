...
function CompletionCodes(props) {
  ...
  const renderRows = React.useMemo(() => {
    if (completionCodes && Array.isArray(completionCodes)) {
      return completionCodes.map((v) => (
        <CompletionCodesItem key={v.id} data={v} callback={handleChangeValue} />
      ));
    }
    return null;
  ...
  React.useEffect(
    ...
    [settingsStoreGetCompletionCodesCancel, settingsStoreUpdateCompletionCodeCancel]
  );
          ...
          <Table selectable celled fixed size="small" unstackable>
            <Table.Header>
              <Table.Row>
                ...
                <Table.HeaderCell width={tableWidth < 1250 ? 2 : 1}>
                  Понятая трубка
                </Table.HeaderCell>
