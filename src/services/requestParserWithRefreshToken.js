...
import { qs } from '@constants/index';
...
const getErrorCode = (error) =>
  error && error.response && error.response.status ? error.response.status : null;

const requestParserWithRefreshToken = async (props) => {
  const { method, url, data, params, headers, requestConfig, repeated } = props || {};
  const config = {
    headers: {
    ...
    if (getErrorCode(e) === 401) {
      ...
      if (rToken) {
        try {
          ...
          const { data: refreshResData } = refreshRes || {};
          const { access_token: token, refresh_token: refreshToken } = refreshResData || {};
