...
function UIInput(props) {
  ...
  return (
    <div className={className}>
      {title
      && (
        <div className="ui-input-v2__title-wrapper">
          <div className="ui-input-v2__title">
            <span className="ellipsis-element">
              {title}
            </span>
            {required && !readOnly && <div className="required-icon">*</div>}
            {hint && !readOnly && (
              <div className="ui-input-v2__hint">
                <Popup
                  content={hintText}
                  trigger={<div className="ui-input-v2__hint-icon">{hintIcon || <HintIcon />}</div>}
                  basic
                />
              </div>
            )}
      ...
      <div className="ui-input-v2__body" ref={elRef}>
        {renderBody}
        {!disabled && !readOnly && (
        <div role="presentation" className="ui-input-v2__clear" title="Очистить" onClick={handleClear}>
          <ClearIcon />
        </div>
        )}
        {!readOnly && !disabled && (
          <>
            <div className="ui-input-v2__error" title="Ошибка в поле">
              <ErrorIcon />
            </div>
            <div className="ui-input-v2__success" title="Поле заполнено верно">
              <SuccessIcon />
            </div>
          </>
        )}
        {successFormat && <div className="ui-input-v2__i-error">{successFormat}</div>}
      </div>
    </div>
  );
}
