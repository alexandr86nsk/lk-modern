...
function UITable(props) {
  ...
  React.useEffect(() => setCurrentPage(1), [itemsPerPage, searchString, filterSelected]);
  ...
  const handleSetNumber = React.useCallback((value) => setItemsPerPage(value), [setItemsPerPage]);
          ...
          Object.keys(v).filter((key) => {
            if (v[key] && header.filter((w) => w.value === key).length) {
              if (header.filter((x) => x.value === key)[0].type === 'options-select') {
                if (
                  header
                    .filter((y) => y.value === key)[0]
                    .options.filter((z) => z.value === v[key])[0]
                ) {
                  return header
                    .filter((y) => y.value === key)[0]
                    .options.filter((z) => z.value === v[key])[0]
              ...
              if (header.filter((x) => x.value === key)[0].type === 'date') {
                return moment(v[key])
                  .format(v.dateFormat || 'L')
                  .includes(searchString.toLowerCase());
              }
              if (Array.isArray(v[key])) {
                return v[key].filter((b) =>
                  b[header.filter((x) => x.value === key)[0].searchValue]
                    .toString()
                    .toLowerCase()
                    .includes(searchString.toLowerCase())
                ).length;
  ...
  const indexOfLastItem = React.useMemo(() => currentPage * itemsPerPage, [
    currentPage,
    itemsPerPage,
  ]);
  const indexOfFirstItem = React.useMemo(() => indexOfLastItem - itemsPerPage, [
    indexOfLastItem,
    itemsPerPage,
  ]);
  const itemsToPage = React.useMemo(() => dataToPage.slice(indexOfFirstItem, indexOfLastItem), [
    dataToPage,
    indexOfFirstItem,
    indexOfLastItem,
  ]);
    ...
    () =>
      header.map((v) => {
        if (v.colSpan !== 0) {
          ...
          return (
            <Table.HeaderCell key={v.id} colSpan={v.colSpan}>
              {v.title}
            </Table.HeaderCell>
          );
        }
  ...
  const renderRows = React.useMemo(() => {
    ...
    if (data.length) {
      ...
      result = (
        <Table.Row>
          <Table.Cell className="ui-table__empty" colSpan={header.length}>
            {empty}
          </Table.Cell>
        </Table.Row>
      );
      ...
      {(pagination || search) && (
        <div className="ui-table__controls-block">
          <div className="controls-block__left-block">
            ...
            {data && data.length > 0 && <div className="total-records">{`из ${data.length}`}</div>}
          </div>
            ...
            {filter && (
              <div className="filter-block" ref={filterEl}>
                ...
                >
                  <FilterIcon />
                  {filterSelected > 0 && (
                    <div className="filter-block__badge">{filterSelected}</div>
                  )}
                </div>
                {showFilter && (
                  <UITableFilter
